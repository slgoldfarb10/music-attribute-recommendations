## this program is for getting a cluster of recommended songs for you from a given artist
## based on 1-2 songs you like by them
...
##---------------------------------##
    ...
    ## if you want all singles/eps/nonstandard albums, set this to true
    album_get_alltypes = False
##---------------------------------##

    if album_get_auto:
        if album_get_alltypes :
            ## you may have to set a limit on the number of albums to get if the program times out/throws an error
            albums_data = sp.artist_albums(artist, limit=25)
        else :
...
for i,_df in enumerate(df) :
    ...
    ## using danceability, energy, instrumentalness, and speechiness (recommended)
    X[i] = np.matrix(np.dstack((f2,f3,f4,f9)))

    ## using acousticness, danceability, energy, and speechiness (alternate)
#    X[i] = np.matrix(np.dstack((f1,f2,f3,f9)))

## shifts X into Z, a matrix that the clustering function can process
Y = []

for mat_x in X :
    Y.extend(mat_x.tolist())
...
##---------------------------------##
    ## choose whether to cluster based on one song or two, and input indices of songs you want
two_song = True
goodsong1 = 0 # first song index here
goodsong2 = 1 # second song index here (if two_song = True)
##---------------------------------##

num_clusters = 1

## if you want more of fewer songs in your final cluster, change this value
min_matches = 4
...
if two_song :
    ...
    while keepClustering :
        curr_clusters = num_clusters
        for x in range(0, 10):
            fitCluster = KMeans(n_clusters = num_clusters).fit(Z)
            song1Cluster = fitCluster.labels_[goodsong1]
            if (song1Cluster == fitCluster.labels_[goodsong2]) :
                if (fitCluster.labels_.tolist().count(song1Cluster) < min_matches) :
                    break
                num_clusters += 1
                goodCluster = fitCluster
                break
...
if (goodCluster == []) :
    print ("could not cluster your songs")
else :
    clusterList = goodCluster.labels_
    myCluster = clusterList[goodsong1]
    mySongs = []
    for i, v in enumerate(clusterList) :
        if v == myCluster :
            mySongs.append(i)

    finalNames = []
    for x in mySongs :
        finalNames.append(names[x])

    ## use this output to get the songs in your recommended cluster
    print(finalNames)
